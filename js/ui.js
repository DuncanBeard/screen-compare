...
const UI = {
  ...
  createResultsHtml(metrics, screen) {
    const ppiColor =
      metrics.ppiRating === "high"
        ? "var(--ppi-high)"
        : metrics.ppiRating === "medium"
        ? "var(--ppi-medium)"
        : "var(--ppi-low)";
    ...
    for (const name of names) {
      ...
      item.innerHTML = `
                <div class="saved-item-info">
                    ...
                    <span class="saved-item-meta">${screenCount} screen${
        screenCount !== 1 ? "s" : ""
      } - ${savedDate}</span>
                </div>
  ...
  renderAutoArrange(canvas, screenData, arrangement) {
    ...
    screenData.forEach((data, displayIndex) => {
      ...
      const rect = this.createScreenRect(screen, dims, colors, isDigital, true);
  ...
  // Store action callbacks for screen rects
  screenActions: null,

  /**
   * Set action callbacks for screen rect buttons
   */
  ...
  /**
   * Start inline editing of a screen label
   * @param {HTMLElement} label - The label element
   * @param {string} screenId - The screen ID
   */
  startLabelEdit(label, screenId) {
    const currentName = label.textContent;
    const input = document.createElement("input");
    input.type = "text";
    input.value = currentName;
    input.className = "screen-rect-label-input";
    ...
    const finishEdit = (save) => {
      ...
      if (save && newName && newName !== currentName && this.screenActions.onRename) {
        this.screenActions.onRename(screenId, newName);
      } else {
   ...
   */
  createScreenRect(screen, dims, colors, isDigital = false, showActions = false) {
    ...
    const actionsHtml = showActions
      ? `
            <div class="screen-rect-actions">
                <button class="screen-action-btn" data-action="rotate" title="Rotate 90°">↻</button>
                <button class="screen-action-btn" data-action="duplicate" title="Duplicate">⧉</button>
                <button class="screen-action-btn" data-action="delete" title="Delete">🗑</button>
            </div>
        `
      : "";

    if (sizeLabel || showActions) {
      rect.innerHTML = `
                ${actionsHtml}
                ${
                  sizeLabel
                    ? `<span class="screen-rect-label" title="Click to edit name">${this.escapeHtml(
                        screen.name
                      )}</span>`
                    : ""
                }
                ${sizeLabel ? `<span class="screen-rect-size">${sizeLabel}</span>` : ""}
            `;
    }

    // Add click-to-edit functionality for the label
    if (sizeLabel && this.screenActions && this.screenActions.onRename) {
      const label = rect.querySelector(".screen-rect-label");
      if (label) {
        label.addEventListener("click", (e) => {
          e.stopPropagation();
          this.startLabelEdit(label, screen.id);
        });
    ...
    if (showActions && this.screenActions) {
      rect.querySelectorAll(".screen-action-btn").forEach((btn) => {
        btn.addEventListener("click", (e) => {
          e.stopPropagation();
          const action = btn.dataset.action;
          if (action === "delete" && this.screenActions.onDelete) {
            this.screenActions.onDelete(screen.id);
          } else if (action === "duplicate" && this.screenActions.onDuplicate) {
            this.screenActions.onDuplicate(screen.id);
          } else if (action === "rotate" && this.screenActions.onRotate) {
            this.screenActions.onRotate(screen.id);
          }
        });
